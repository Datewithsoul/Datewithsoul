  ...
  if (!user) {
    return (
      <div className="flex items-center gap-2">
        <Link
          href="/login"
          className="text-sm font-semibold text-gray-700 hover:text-black transition-colors px-3 py-1.5 rounded-lg hover:bg-gray-100"
        >
          เข้าสู่ระบบ
        </Link>
        <Link
          href="/register"
          className="text-sm font-semibold bg-[#222222] text-white px-4 py-2 rounded-full hover:bg-black transition-colors"
        >
          สมัครสมาชิก
        </Link>
      </div>
        ...
        <div className="absolute right-0 mt-3 w-56 bg-white border border-gray-200 rounded-xl shadow-lg py-2 z-50 overflow-hidden">
          ...
          {isAdmin && (
            <Link
              href="/admin"
              onClick={() => setIsOpen(false)}
              className="block px-4 py-2 text-sm font-semibold text-[#F44336] hover:bg-red-50 transition-colors mt-1"
            >
              หน้าจัดการ (Admin)
            </Link>
          )}
          ...
          <button
            ...
            className="w-full text-left block px-4 py-2 text-sm hover:bg-gray-50 text-gray-700 hover:text-black transition-colors"
          >
            ออกจากระบบ
          </button>
        </div>
